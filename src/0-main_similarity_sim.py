...
def get_similarity2(method, clean_summary_dir, clean_comment_cut_dir, matrix_similarity_dir):
    ...
    for idx, file_name_txt in enumerate(pathDir):
        ...
        y_pred.extend(get_path(similarity_list))
        print(len(y_pred))
    ...
    resu.append({"P": str(all_P / len(pathDir)), "R": str(all_R / len(pathDir)), "F": str(all_F / len(pathDir))})
    datas.append(resu)

    matrix_similarity = os.path.join(matrix_similarity_dir, "all_character_only_" + method)
...
def get_similarity(clean_summary_dir, clean_comment_cut_dir, matrix_similarity_dir):
    ...
    for idx, file_name_txt in enumerate(pathDir):
        ...
        if int(danmu_comment) != 0:
            ...
            y_pred = get_path(similarity_list)
            print(len(y_pred))
    ...
    dump_to_json(datas, open(matrix_similarity, 'w', encoding='utf8'))
    print("---------finished---------")
