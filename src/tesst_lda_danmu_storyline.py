...
def process_danmus(summary_dir, data_sava_path, origin_comment_dir, dicts=None):
    ...
    for idx, file_name_txt in enumerate(pathDir):
        ...
        for i, n in enumerate(summary):
            video_time_slice.append(cut_num*(i+1))
        cut_num_init = 0
        for mm, n in enumerate(summary):
            danmu_cut = ""
            ps_init = 0
            for i,m in enumerate(danmu):
                ...
                if i <= cut_num * (cut_num_init + 1):
                    ...
                    info.append(danmu_cut)



                    # "playtime": "11000", "video": "314646808"
                    # 头部
                    dt = []

                    dt.append(danmu_cut)
                    #dt.append(m["playtime"])
                    #dt.append(m["video"])
                    dt.append(cut_num_init)
                    ...
                    cut_num_init += 1
            info.append(n["content"])

        data_sava_path = data_sava_path + danmu_file[0] +".csv"
...
if __name__ == '__main__':
    ...
    data_sava_path = "../data/cutldapair_labelsdddss"
