...
def process_danmus(origin_comment_dir, data_sava_path, summary_dir, dicts=None):
    ...
    for idx, file_name_txt in enumerate(summary_path_dir):
        ...
        for i in range(1):
            ...
            dt.append(
                danmu_all_list[i].get("danmaku") + danmu_all_list[i + 1].get("danmaku")
                + danmu_all_list[i + 2].get("danmaku") + danmu_all_list[i + 3].get("danmaku")
                + danmu_all_list[i + 4].get("danmaku") + danmu_all_list[i + 5].get("danmaku") + danmu_all_list[
                    i + 6].get(
                    "danmaku")
                + danmu_all_list[i + 7].get("danmaku") + danmu_all_list[i + 8].get("danmaku")
                + danmu_all_list[i + 9].get("danmaku")
            )
            ...
            dt.append(danmu_all_list[d_len - i].get("danmaku") + danmu_all_list[d_len - i - 1].get("danmaku") +
                      danmu_all_list[d_len - i - 2].get("danmaku") + danmu_all_list[i - 3].get("danmaku")
                      + danmu_all_list[i - 4].get("danmaku") + danmu_all_list[d_len - 5].get("danmaku") +
                      danmu_all_list[d_len - 6].get("danmaku") +
                      danmu_all_list[d_len - 7].get("danmaku") + danmu_all_list[i - 8].get("danmaku")
                      + danmu_all_list[i - 9].get("danmaku"))
            dt.append("1")
            info.append(dt)
...
if __name__ == '__main__':
    ...
    data_sava_path = "../data/danmucutpair_label10_pos.csv"
    process_danmus(origin_comment_dir, data_sava_path, summary_dir)
    print("---finished---")
