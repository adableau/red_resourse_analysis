...
def save_test(total_similarity):
    data_sava_path = "../data/character.csv"
    with open(data_sava_path, "w", encoding="UTF-8", newline="") as csvfile:  ##newline=""去除空白行，又叫换行！
        # csvfile.write(codecs.BOM_UTF8)  ##存入表内的文字格式
        writer = csv.writer(csvfile)  # 存入表时所使用的格式
        writer.writerow(['similarity'])
        writer.writerows(total_similarity)  # 写入表
...
def get_similarity(method, clean_summary_dir, clean_comment_cut_dir, matrix_similarity_dir):
    ...
    for idx, file_name_txt in enumerate(pathDir):
        ...
        for n in summary:
            ...
            for m in danmu:
                ...
                if ps_init == m["cut"]:
                    ...
                    if method == "jaccard_similarity":
                        jjs = jsm.character_jaccard_similarity(danmu_cut, n["content_clean"])
            ...
            allsimilarity_list.append(s_score)
        ...
        print(len(y_true))
        # y_pred = np.array(similarity_list)
    ...
    save_test(allsimilarity_list)
    resu.append({"P": str(all_P / len(pathDir)), "R": str(all_R / len(pathDir)), "F": str(all_F2 / len(pathDir)),
                 "micro_prf": str(all_F / len(pathDir))})
    datas.append(resu)

    matrix_similarity = os.path.join(matrix_similarity_dir, "argmax_word_DP_" + method)
