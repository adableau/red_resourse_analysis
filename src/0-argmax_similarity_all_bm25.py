...
def get_similarity(method, clean_summary_dir, clean_comment_cut_dir, matrix_similarity_dir):
    ...
    for idx, file_name_txt in enumerate(pathDir):
        ...
        danmus = file_utils.load_danmu(danmu_file_path)
        flags = 0
        ...
        for n in summary:
            words = list(n["content_clean"])
            doc.append(words)

        bm_model = bm25.BM25(doc)

        print(bms.cal_bm25_sim(bm_model, "人能", '人工智能计算机科学'))
            ...
            for m in danmus:
                ...
                if ps_init == m["cut"]:
                    ...
                    if method == "jaccard_similarity":
                        ...
                        jjs = bms.cal_bm25_sim(bm_model, danmu_cut, n["content_clean"])
    ...
    matrix_similarity = os.path.join(matrix_similarity_dir, "argmax_bm25_only_" + method)
