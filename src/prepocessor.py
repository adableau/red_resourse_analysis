            ...
            with open(origin_path, 'r', encoding='utf8') as fr:
                ...
                for line in fr:
                    ...
                    if len(content_list) > 10 and len(content_list[9]) > 3:  # 第10列danmaku
                        ...
                        comment = process(content_list[9])  # 分詞加dict
                        if comment not in danmakus:
                            datas.append({'cut': int(i / 30), 'danmaku': " ".join(comment), 'video': content_list[10],
                                          'playtime': content_list[6]})
...
if __name__ == '__main__':
    ...
    clean_comment_dir = '../data/new_clean_danmu_/'
